...
func TestBuildOIDCCallbackURL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := buildOIDCCallbackURL(req, tt.provider)
...
// newOIDCTestRequest builds a plain httptest request with the given chi
// route param ("provider") wired into the request context, mirroring how
// chi's router populates URL params at request time.
func newOIDCTestRequest(target, provider string) (*http.Request, *httptest.ResponseRecorder) {
...
// every branch via middleware.RenderHTML, which is nil until main.go wires it
// up at startup, so this test only proves the guard clause is exercised
...
func TestOIDCAuthHandlerStartLogin_Disabled(t *testing.T) {
	...
	r, w := newOIDCTestRequest("/server/auth/oidc/okta/start", "okta")
...
func TestOIDCAuthHandlerCallback_Disabled(t *testing.T) {
	...
	r, w := newOIDCTestRequest("/server/auth/oidc/okta/callback", "okta")
